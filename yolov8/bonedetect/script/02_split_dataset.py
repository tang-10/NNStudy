...
BASE_PATH = r"/home/tangl/projects/datasets/bone"
# 数据集路径
...
def main():
    ...
    for i in list_all_txt:
        ...
        srcImage = image_original_path + name + image_suffix
        srcLabel = label_original_path + name + ".txt"

        if i in train:
            dst_train_Image = train_image_path + name + image_suffix
            dst_train_Label = train_label_path + name + ".txt"
            shutil.copyfile(srcImage, dst_train_Image)
            shutil.copyfile(srcLabel, dst_train_Label)
        elif i in val:
            dst_val_Image = val_image_path + name + image_suffix
            dst_val_Label = val_label_path + name + ".txt"
            shutil.copyfile(srcImage, dst_val_Image)
            shutil.copyfile(srcLabel, dst_val_Label)
        else:
            dst_test_Image = test_image_path + name + image_suffix
