...
def get_imgwh(xml_file):
    img_path = xml_file.replace(
        "Fleurine_frames_annotations", "Fleurine_frames"
    ).replace(".xml", image_suffix)
...
if __name__ == "__main__":
    ...
    image_suffix = ".jpg"
    # 需要转换的类别，需要一一对应
    classes = ["Frieren", "Fern", "Stark", "Himmel", "Heiter", "Eisen", "Wirbel"]
    # 2、voc格式的xml标签文件路径
    xml_files = os.path.join(BASE_PATH, "Fleurine_frames_annotations")
